...
//		datasets.add(Table.readCSV("datasets/http.csv",        ",",  3, true));
		...
		parameters.add(new Quartet<>("parkinson  ", 0.7482,    1,  30));
//		parameters.add(new Quartet<>("hepatitis  ", 0.5190,    1,  10));
//		parameters.add(new Quartet<>("wine       ", 0.5530,    27, 10));
//		parameters.add(new Quartet<>("glass      ", 0.0758,    4,  10));
//		parameters.add(new Quartet<>("vertebral  ", 0.4711,   82,  100));
...
//				for (double r = .0001; r <= .1; r += .0001) {
					long start = System.currentTimeMillis();
					double[] output = SSJ(X, r/ratio, outThreshold, joinThreshold);
					long end = System.currentTimeMillis();
					double runtime = (end - start) / 1000.0;
					double auc = AUC.measure(labels, output);

//					System.out.println(
//							String.format("dataset=%s\tjoinThs=%d\toutThs=%d\trange=%.4f\tauc=%.4f\truntime=%.2f",
//									dataset, joinThreshold, outThreshold, r, auc, runtime));
					System.out.println(String.format("%s,%d,%d,%.4f,%.4f,%.3f", dataset, joinThreshold, outThreshold, r/ratio,
							auc, runtime));
				}
			}
		}
